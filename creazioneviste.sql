...
select * from [dwh].[vBC_region_SDI1]
DROP VIEW IF EXISTS [dwh].[vBC_region_SDI1];
create view [dwh].[vBC_region_SDI1] as
select 
CASE 
        WHEN CHARINDEX('(', CAST([measure] AS NVARCHAR(40))) > 0 THEN 
            SUBSTRING(CAST([measure] AS NVARCHAR(40)), 1, CHARINDEX('(', CAST([measure] AS NVARCHAR(40))) - 1)
...
select * from [dwh].[vBC_region_SEX1]
...
select * from [dwh].[vIHME_POP_2017_2100_POP_REFERENC]
...
UPDATE [dwh].[GHDX_Deaths_CLEAN2]
SET [location] = REPLACE([location], 'Sao Tome and Principe', 'São Tomé and PrÍncipe')
...
GO
CREATE VIEW [dwh].[vGHDX_SDI_Countries_UNPIVOT] AS
SELECT
    [location]
